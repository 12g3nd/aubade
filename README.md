...
The repository includes a GitHub Actions workflow that builds on a hosted macOS runner. Run it manually from the **Actions** tab, or push a change under `native-trial/`. Download the `AubadeTrial-unsigned-ipa` artifact from the completed run, then sign and install it with Sideloadly on Windows.
...
Follow [native-trial/README.md](native-trial/README.md). The current trial checks offline launch, local storage, export, and persistence after re-signing. Free Apple-account signing expires after seven days, so Sideloadly must refresh it from your Windows laptop.
